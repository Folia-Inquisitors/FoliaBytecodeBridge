...
| `Block#getBlockData()` | `UnsafeCallBridge#blockGetBlockData` | `C_REGION_BLOCK` | `region` | `region-scheduler-by-block` | Block-owned block-data read. Mirrors the material-read route and adds `block-data-cache` / `global-scheduler-block-data-cache-miss` diagnostics for synchronous return paths reached after scheduler hops. |
