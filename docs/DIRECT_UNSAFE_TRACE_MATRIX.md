...
| `Player#playSound(...)` | `UnsafeCallBridge#playerPlaySound` | `A_ENTITY` | `player` | `entity-scheduler-audio` | Player audio effect; seen in PlayerKits2 and Essentials command effects. |
...
| `Player#getScoreboard()` | `UnsafeCallBridge#playerGetScoreboard` | `D_PLAYER_UI` | `scoreboard` | `scoreboard-player-owned-read` | Player-owned scoreboard model read; added after TAB/BetterBoard/SuperVanish reference audit. |
...
| `Team#setOption(Option,OptionStatus)` | `UnsafeCallBridge#teamSetOption` | `D_PLAYER_UI` | `scoreboard` | `scoreboard-model-team-mutation` | Team option mutation, seen in Folia-compatible SuperVanish no-push shape. |
