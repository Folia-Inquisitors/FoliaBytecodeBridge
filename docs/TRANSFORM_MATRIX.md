...
| `Block#getType()`, `Block#setType(Material)` | `C_REGION_BLOCK` preemptive owner-check route | Block-owned material read/mutation. The bridge asks whether the current thread owns the block, keeps owned calls direct, schedules unowned reads when waiting is safe, and uses a logged block-material model for foreign owner threads where a synchronous region wait would stall Folia. |
