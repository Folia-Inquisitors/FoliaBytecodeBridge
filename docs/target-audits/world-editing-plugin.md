# world-editing plugin reference Audit Notes

Source reference: `<downloads>\world-editing-plugin-reference.zip`

The source was extracted to a short path for Windows path-length reasons:

```text
<workspace>\world-editing plugin reference-main
```

## Scheduler Calls Covered By The Bridge

world-editing reference mostly funnels Bukkit scheduling through central platform classes. That is good for the bytecode bridge because the transformer only needs to catch a small number of stable Bukkit scheduler call shapes.
...
covered in the raw scheduler transformer as a general `S_ASYNC` bytecode shape, not as a world-editing reference-only
branch. The smoke assertion `rawLegacyMainThreadOwnerAsyncRepeatingHits=1` verifies the actual world-editing reference class rewrites to
`ObjectSchedulerBridge#scheduleAsyncRepeatingTask`.

| File | Pattern | Bridge result |
| --- | --- | --- |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `scheduleSyncRepeatingTask` | global repeating task |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `scheduleAsyncRepeatingTask` | async repeating task |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `runTaskAsynchronously` | async task |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `runTask` | global task |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `runTaskLater` | global delayed task |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `runTaskLaterAsynchronously` | async delayed task |
| `worldedit-bukkit/src/main/java/com/worldeditingreference/bukkit/util/BukkitTaskManager.java` | `cancelTask` | synthetic task cancellation when known |
...
world-editing reference has many direct world, chunk, entity, and block operations. Examples found during the scan include:
...
That failure occurs in world-editing reference's `PaperweightLegacyMainThreadOwnerWorldNativeAccess` adapter before a Bukkit
...
The bridge should help world-editing reference only with legacy Bukkit scheduler entry points. It may reduce immediate scheduler API errors, especially where world-editing reference's `TaskManager` reaches Bukkit's old scheduler.

## Remaining Manual Folia Work

world-editing reference is much deeper than a scheduler compatibility problem. True Folia compatibility needs source-level work around:
...
This bridge is useful as an experiment, but world-editing reference should not be considered Folia-safe just because scheduler bytecode rewrites pass.
