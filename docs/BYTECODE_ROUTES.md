...
The inherited `BukkitRunnable` fallback is deliberately narrower than a plain method-name match. It accepts `org/bukkit/scheduler/BukkitRunnable` directly and javac anonymous-owner names such as `ExampleConfigTask$1`, but it does not rewrite named plugin helper methods such as `ExamplePlugin#runTaskAsynchronously(Runnable)`. The live smoke test proved those helpers can look scheduler-like while using a different receiver shape, so they must be routed through their internal `BukkitScheduler` call instead.
